...
namespace EmbedUtils {
    ...
    // Struttura per rappresentare una libreria embedded
    struct EmbeddedLibrary {
        std::string name;                    // Nome della libreria
        std::vector<uint8_t> data;          // Dati binari
        size_t originalSize;                 // Dimensione originale
        std::string checksum;                // Checksum per verifica integrità
        bool isCompressed;                   // Flag compressione
...
namespace EmbedUtils {
    ...
    // Ottiene nome file da path
    std::string GetFilename(const std::string& filepath);
    
    // Ottiene estensione file
    std::string GetExtension(const std::string& filepath);
    
    // Crea nome variabile valido in C++
