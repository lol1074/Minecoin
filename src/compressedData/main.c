/*
 * SISTEMA DI COMPRESSIONE E CRITTOGRAFIA AVANZATO
 * 
 * Pipeline completa:
 * 1. Chunking (16MB)
 * 2. Pre-processing (BWT + MTF + RLE)
 * 3. Prediction (Context + Delta)
 * 4. Compression (LZ4 + Huffman + ANS)
 * 5. Obfuscation (Polymorphic + Fake + Shuffle)
 * 6. Encryption (Argon2 + AES-256-GCM)
 */
...
ChunkPipelineResult* process_chunk_compress(Chunk *chunk, ProcessContext *ctx) {
    ...
    // STAGE 4: OBFUSCATION
    result->obfuscation = obfuscate_data(current_data, current_size, &ctx->obfuscation);
...
bool process_chunk_decompress(Chunk *chunk, ChunkPipelineResult *pipeline) {
    if (!chunk || !pipeline) return false;
    ...
    if (pipeline->obfuscation) {
        if (!deobfuscate_data(current_data, current_size, pipeline->obfuscation, temp1)) {
    ...
    if (pipeline->prediction) {
        if (!prediction_decode(current_data, current_size, pipeline->prediction, temp2, current_size)) {
            free(temp1); free(temp2); free(temp3);
            return false;
        }
        current_data = temp2;
    ...
    if (pipeline->preprocess) {
        ...
        if (!preprocess_decode(current_data, current_size, pipeline->preprocess, temp3, &decoded_size)) {
            free(temp1); free(temp2); free(temp3);
            return false;
        }
        current_data = temp3;
...
void pipeline_result_free(ChunkPipelineResult *result) {
    ...
    if (result->preprocess) preprocess_result_free(result->preprocess);
    if (result->prediction) prediction_result_free(result->prediction);
    if (result->obfuscation) obfuscation_result_free(result->obfuscation);
...
StatusCode compress_file(const char *input_path, const char *output_path, 
    ...
    ChunkPipelineResult **pipelines = calloc(reader->total_chunks, 
                                            sizeof(ChunkPipelineResult*));
    
    while (chunks_processed < reader->total_chunks) {
        progress_callback(
            (float)chunks_processed / reader->total_chunks * 100.0f,
            "Processing chunks"
        );
